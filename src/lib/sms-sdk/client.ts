...
import { ResultCode, DEFAULT_HOST, DEFAULT_PORT } from "./types";
import {
...
export class EmaySmsClient {
  ...
  private prepareRequest<
    T extends { requestTime?: number; requestValidPeriod?: number },
  >(request: T): T {
    return {
      ...request,
   ...
   */
  async sendSingleSms(
    request: SmsSingleRequest,
  ): Promise<ResultModel<SmsResponse>> {
   ...
   * ```
   */
  async sendBatchOnlySms(
    request: SmsBatchOnlyRequest,
  ): Promise<ResultModel<SmsResponse[]>> {
    const preparedRequest = this.prepareRequest(request);
    return sendEncryptedRequest<SmsResponse[]>(
      this.appId,
   ...
   * ```
   */
  async sendPersonalityAllSms(
    request: SmsPersonalityAllRequest,
  ): Promise<ResultModel<SmsResponse[]>> {
    const preparedRequest = this.prepareRequest(request);
    return sendEncryptedRequest<SmsResponse[]>(
      this.appId,
  ...
  ): Promise<ResultModel<BalanceResponse>> {
    const preparedRequest = this.prepareRequest(request);
  ...
  ): Promise<ResultModel<ReportResponse[]>> {
  ...
  ): Promise<ResultModel<RetrieveReportResponse>> {
